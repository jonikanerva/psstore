...
- **`AGENTS.md §3.2` anti-boilerplate rule + the DI guardrail**: no `ViewModel` / `Service` / `Controller` per tiny view; and no architecture astronautics _on top of_ Effect — services are flat (one `Layer` per external system: Sony client, cache, clock), no `Layer` that only composes other `Layer`s, no generic service factories.
