...
| 2026-05-29 | §5 UI responsiveness / §6 Side effects (full result set) | UPCOMING is bounded to a ~12 SKU-bearing concept ceiling | The `next_thirty_days` facet returns ~53 announced concepts, but ~41 are pure announcements with `products: []` / `price: null` and no PDP, so they have no SKU/date/price and are correctly dropped by `PRODUCT_ID_PATTERN` + the empty-date filter. UPCOMING therefore surfaces only the ~12 purchasable, dated concepts. Widening it would require a non-anonymous surface for announced-but-not-purchasable titles — out of scope for the anonymous proxy. |
